...
⚠️ **再补记：[Avatar-Forever](../avatar_forever/analysis.md)（PolyU + ByteDance，音频驱动数字人）给出了第四种、也最激进的答案 —— ① 和 ② 都不要。** 它的 Fig 1 上半画的正是这条流水线（Teacher Forcing → AR Init → ODE Init → DMD），然后把它拆成两路从同一个 22B LTX-2.3 出发的**并行**训练：一路全参 DMD（30 → 4 步，**不带任何 rollout、不碰历史上下文**），一路 rank-128 的视频侧 LoRA 专管长时（RRT：扰动最早的历史 chunk → 30 步 base 模型无梯度 rollout 4 个 chunk → 只在第 5 个真值 chunk 上算 flow matching），部署时 `θ★ = θ0 + Δθ_DMD + Δθ_RRT`。
