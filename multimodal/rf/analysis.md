...
> Fig 1：(a) VAE-based UMM：生成路径强依赖冻结 VAE；(b) Naive pixel-space：去掉 VAE 但无结构引导，生成崩坏；(c) Representation Forcing：解码器先 AR 预测 rep token（来自自身的理解编码器），再用这些 token in-context 引导像素扩散，完全无需外部 latent space。
...
> Fig 3：左侧（训练流程）—— 解码器处理统一 token 序列 [T, R, P]，T 和 R 用因果 AR 预测，P 用双向扩散；右侧（Rep token 提取）—— EMA 编码器从 GT 图像提取特征，经在线 VQ 离散化为 rep token，提供 `L_Rep` 的训练目标和 R 位置的 teacher-forcing 输入。推理时右侧完全绕过，解码器自己预测 rep token。
...
> Fig 2：RF-Pixel 在 1024×1024 分辨率的文生图效果——动物、人像、食物、场景均细节丰富、结构连贯，全程无 VAE。
...
> Fig 4：上行 without RF——猫、鸭子、台阶、水管等物体结构扭曲、构图混乱；下行 with RF——相同 prompt 生成结构连贯、细节清晰，说明 rep token 为像素扩散提供了有效的高层语义骨架。
