...
几个设计要点:
- **Pose 用相对变换(delta)**:`ΔT_t = T_{t-1}^{-1} · T_t`,避免依赖具体 PID 参数或 actuator 接口
...
$$
z = W^{(k)}_{\text{in}} \cdot x + b^{(k)}_{\text{in}}, \quad x = W^{(k)}_{\text{out}} \cdot z + b^{(k)}_{\text{out}}
$$

每个 embodiment `k` 一对 `(W_in^(k), W_out^(k))`,**MoT backbone 完全共享**。6D rotation 解码时用 SVD 还原成 3×3 SO(3)。
...
| **T2I** | `[S_AR, v~_1]` |
| **T2V (+Audio)** | `[S_AR, v~_{1:N}, s~]` |
| **I2V / V2V (+Audio)** | `[S_AR, v_{1:P}, v~_{P+1:N}]`,前 P 帧 clean conditioning |
| **Video Transfer** | `[S_AR, v^ctrl_{1:N}, v~_{1:N}]`,depth / edge / seg 当 control |
| **Action**(3 子模式) | Forward Dyn / Inverse Dyn / Policy(见下图) |

其中 `S_AR = [l_1, ..., l_n, [EOS], [BOG]]`,`v~` 表示 noisy(论文里写作 `ṽ` 即带波浪号),`[BOG]` 是 "begin-of-generation" 特殊 token。

![Fig 4: Action 三种模式](./figures/fig4_action_modes.png)

> Fig 4:**同一段 (video, action) 数据**,纯靠选择"哪个 token clean 哪个 noisy"切出三种训练任务。图例:橙色实线 = clean video,橙色虚线 = noisy video,绿色实线 = clean action,绿色虚线 = noisy action。三个 case 共用一个 local 时间窗口 `{v_{t-1}, v_t, v_{t+1}, a_t, a_{t+1}}`,其中 `a_t` 连接 `v_{t-1} → v_t`:
> - **Forward Dynamics**(左):动作全 clean,**未来两帧视频 `ṽ_t, ṽ_{t+1}` 是 noisy 目标** → 给定动作,模型学"未来视频应该长什么样"
> - **Inverse Dynamics**(中):视频全 clean,**两个动作 `ã_t, ã_{t+1}` 是 noisy 目标** → 给定连续视频,反推动作序列
...
>   - **AR subsequence**:`v_1^AR, v_2^AR, ..., l_1, l_2, ..., [EOS], [BOG]`(ViT 视觉 token + 语言 token + 两个特殊 token 收尾)
>   - **DM subsequence**:`ṽ_1^DM, ṽ_2^DM, ..., s̃_1, s̃_2, ..., ã_1, ã_2, ...`(VAE 视觉 + 音频 + 动作 token,波浪号表示加噪)
...
>   - `O_AR = Attn(Q_AR, K_AR, V_AR)` —— 只在 AR 内部 self-attend
>   - `O_DM = Attn(Q_DM, [K_AR; K_DM], [V_AR; V_DM])` —— DM 看 AR 也看自己
...
> - **Video**:`t ∈ {7, 7, 7, 7, 8, 8, 8, 8}`(同帧 4 个 token 共享 t),`h ∈ {0, 0, 1, 1}`、`w ∈ {0, 1, 0, 1}`(2×2 spatial 网格)
...
选基准 `TPS_base = 24/4 = 6`(24 FPS 视频对应)。每个 token 的实际 temporal increment:

$$
\delta t = \frac{\mathrm{TPS}_{\mathrm{base}}}{\mathrm{TPS}}
$$

→ 16 FPS 视频 `δ = 1.5`(走快 50%);30 FPS 视频 `δ = 0.8`(走慢 20%)。同样 1 秒真实时长,无论用 16/24/30 FPS 编码,**RoPE 上占同样的 position 范围**。
...
Per-modality time sampling:每模态独立采 σ。图/音/动作用 **logit-normal**,视频用 **mode sampling**。再过一次 shift reparameterization:

$$
\sigma = \frac{s \cdot \bar{t}}{1 + (s - 1) \cdot \bar{t}}, \quad \bar{t} = 1 - t
$$

`s ≥ 1` 偏向高噪声。`s` 按分辨率调:256p `s = 1`,480p `s = 3`,720p `s = 5`(pre-train),mid-train 涨到 3 / 5 / 10。
...
>
> Noise shift 用 rectified-flow shift 公式 `σ = s·t̄ / (1+(s-1)·t̄)`,`s` 越大 σ 分布越偏高噪声,**分辨率越高、信号越复杂,需要把训练时间多分配到高噪声区段**(高噪声是"打底"阶段,决定大尺度结构)。
>
...
**4 种训练模式**(按 `T_cond` 区分,采样比例 20:56:16:8):
- **T2I**:`T = 1`,纯 image
- **T2V**:`T_cond = 0`,全 noisy
- **I2V**:`T_cond = 1`,首帧 clean
- **V2V**:`T_cond = 2`,前 5 帧(2 个 latent 帧)clean
...
>
> 2. **Per-stream buffers(中左,虚线框内)**:每个 stream 在每个 rank 都有自己的 prefetch buffer `Q_k^(ρ) = [x_1, x_2, x_3, ...]`,异步预取藏 I/O 延迟。这一步保证"流水线不饿死"
>
...
>    - 实测**单这一项提升训练吞吐 54%**
>
> 4. **Packed batch(最右红块)**:rank ρ 最终输出的 `B_i^(ρ)`,两个 budget 约束:
>    - `tokens ≤ T_max`(主要约束,= 74,000)
>    - `samples ≤ N_max`(防御性约束,防止单 batch 塞太多样本)
>
> 结合 look-ahead packing(Fig 13,本笔记没截入):**额外 +8% effective sequence length**。两者叠加是 Cosmos 3 能在 5 模态 + 2048 GPU 上稳跑的工程根基。

四个核心机制:
1. **Token-budgeted packing**:严格 token budget `T_max`,greedy pack,**完全不 padding**
