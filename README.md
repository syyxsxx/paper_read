...
| [video_generation](./video_generation/) | Mask Forcing: Improving Autoregressive Video Diffusion Distillation via Dual-Noise Masking Rollout | [mask_forcing](./video_generation/mask_forcing/analysis.md) | HKUST+LIGHTSPEED, 2026-09 | [project](https://alicezrzhao.github.io/mask_forcing/) | Dual-Noise Masking Rollout：在 AR 视频 DMD self-rollout 每步去噪输入中，随机掩码注入低噪声 token（per-frame×per-chunk 独立采样），使学生轨迹覆盖教师更多模式（KL 分解：marginal KL ≤ 平均条件 KL − 互信息 `I_θ`）；无需真实数据、无需额外训练阶段；Self/Causal/LongLive Forcing 上 HPSv3+0.29~1.03，人类评估 79–83% 偏好 |
